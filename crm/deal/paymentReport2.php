...
<script>
    ...
    function fillBody(){
        ...
        for(var i=0;i<body1.length;i++){
            ...
            // tr +="<td>"+body1[i]["deal_info"]["UF_CRM_1702018321416"]+"</td>";
            // tr +="<td>"+body1[i]["deal_info"]["UF_CRM_1702650885089"]+"</td>";
            // tr +="<td>"+body1[i]["deal_info"]["UF_CRM_1702650778297"]+"</td>";
            // tr +="<td>"+body1[i]["deal_info"]["UF_CRM_1693385814530"]+"</td>";
            // tr +="<td>"+body1[i]["product"][0]["BEDROOMS"]+"</td>";


//             if (body1[i]["deal_info"]["UF_CRM_1707309740747"] == 1) {
//     tr += "<td>Yes</td>";
// } else {
//     tr += "<td>No</td>";
// }
...
// }
    ...
    for (let j = 0; j < tableSelect.rows[i].cells.length; j++) {
        ...
        if (i >= 2 && j === 11) { // Assuming UF_CRM_1702018321416 is in column index 11
            let number = parseFloat(cellText.replace(",", ""));

            // If it's a valid number, add it to the row, otherwise add the string value
            if (!isNaN(number)) {
                row.push(number); // Push the number if valid
            } else {
                row.push(cellText); // Push the original string value (like C1, C2, etc.)
            }
        } else if (i >= 2 && j > 5) {
            let cellText = tableSelect.rows[i].cells[j].innerText.trim();
            let number = parseFloat(cellText.replace(",", ""));
            ...
            if (!isNaN(number)) {
                ...
                row.push(cellText); // For non-numeric values, retain them as strings
            }
        } else {
            row.push(cellText); // Handle non-numeric cells as strings
        }
    }
    ws_data.push(row);
    row = [];
