...
// UF_CRM_1684835285973 - gadaxdis piroba
// UF_CRM_1667309488937 - xelshekrebis gaformis tarigi
...
function getDaricxvebiDaGadaxdebi($fromDate, $toDate, $deals_IDs, $deals_data){
    ...
    while ($ob = $res->GetNextElement()) {
        ...
        // Remove trailing spaces, leading spaces, line breaks, tabs, etc.
        $dealID = trim($arProps["DEAL"]["VALUE"]);

        // If after trimming it's still empty, skip the element
        if ($dealID === "") {
            continue;
        }

        $amount = (float) str_replace("|USD","",$arProps["TANXA"]["VALUE"]);
        $deals_data[$dealID]["payment"] += $amount;
...
// Add versions with trailing spaces
$withTrailing = array_map(fn($v) => $v . " ", $baseIDs);

// Add versions with leading spaces (just in case)
$withLeading = array_map(fn($v) => " " . $v, $baseIDs);

// Merge all variants
$filterDealIDs = array_merge($baseIDs, $withTrailing, $withLeading);

list($daricxvebi, $gadaxdebi, $deals_data) = getDaricxvebiDaGadaxdebi($fromDate, $toDate, $filterDealIDs, $deals_data);
...
$dataForAllReports = $dataForJson;

if (isset($_GET['format']) && $_GET['format'] === 'json') {
    $dealIds = explode(',', $_GET['dealIds']);

    $fromDate = DateTime::createFromFormat('Y-m-d', $_GET['from_date']);
    $monthKey = $fromDate->format('Y-m'); // e.g. 2025-09

    foreach($dealsForExcel as $dealId => $deal) {
        if (in_array($deal["ID"], $dealIds)) {
            $dataForJson[$deal["ID"]] = $dealsForExcel[$dealId]["gadaxdebi_and_daricxvebi_by_dates"][$monthKey]["gadaxda"];
        }
    }

    ob_end_clean();
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode($dataForJson, JSON_UNESCAPED_UNICODE);
    exit();
}
ob_end_clean();
?>
