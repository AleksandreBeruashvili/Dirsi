...
foreach ($filteredProducts as $product) {
    ...
    if ($product["KORPUSIS_NOMERI_XE3NX2"] === "P") {
        $resArray["გარე ავტოსადგომი"][$prodStatus]["num"]++;
        $resArray["გარე ავტოსადგომი"][$prodStatus]["total_area"] += (float) $product["TOTAL_AREA"] ?? 0;
        $resArray["გარე ავტოსადგომი"][$prodStatus]["price"] += (float) $product["PRICE"] ?? 0;
    } else if ($product["PRODUCT_TYPE"] === $prodType) {
        if ($product["PRODUCT_TYPE"] === "ავტოსადგომი") {
        ...
        if ($product["PRODUCT_TYPE"] === "ბინა") {
            if ($product["Bedrooms"] === "1") {
                $prodTypeAnothaOne = "ბინა (1 საძ.)";
            } else if ($product["Bedrooms"] === "2") {
                $prodTypeAnothaOne = "ბინა (2 საძ.)";
            } else if ($product["Bedrooms"] === "3") {
                $prodTypeAnothaOne = "ბინა (3 საძ.)";
            } else {
                continue;
            }
    ...
    <tbody>
        ...
        <tr <?= $isSubRow ? 'class="sub-row"' : '' ?>>
            ...
            <?php foreach (['თავისუფალი','დაჯავშნილი','გაყიდული','გადაცემული','NFS'] as $s):
                $val = $infos[$s]['price'] ?? 0;
                $num = $infos[$s]['num']   ?? 0;
                $avg = $num > 0 ? '$' . number_format($val / $num, 2) : '—';
            ?>
            <td><?= $avg ?></td>
            <?php endforeach; ?>
            <td style="font-weight:bold"><?= $total_num > 0 ? '$' . number_format($total_price / $total_num, 2) : '—' ?></td>
        </tr>
        <?php endforeach; ?>
        <tr class="total-row">
            <td><?= $t['col_total'] ?></td>
            <?php foreach (['თავისუფალი','დაჯავშნილი','გაყიდული','გადაცემული','NFS'] as $s):
                $val = $status_totals_price[$s];
                $num = $status_totals_num[$s];
                $avg = $num > 0 ? '$' . number_format($val / $num, 2) : '—';
            ?>
            <td><?= $avg ?></td>
            <?php endforeach; ?>
            <?php
                $grand_total_price = array_sum($status_totals_price);
                $grand_total_num   = array_sum($status_totals_num);
            ?>
            <td><?= $grand_total_num > 0 ? '$' . number_format($grand_total_price / $grand_total_num, 2) : '—' ?></td>
        </tr>
    </tbody>
</table>
