...
// Custom "კატალოგი" tab for deals
global $APPLICATION;
$curPage = $APPLICATION->GetCurPage();
$url = explode("/", $curPage);

// Check if we are viewing a Deal
if ($url[2] == "deal" && intval($url[4]) > 0) {
    $dealId = intval($url[4]);
    ...
    array_push($tabs, $catalogTab);
}

// Check if we are viewing a Deal 2 test
// if ($url[2] == "deal" && intval($url[4]) > 0) {
//     $dealId = intval($url[4]);

//     $catalogTabTest = array(
//         "id" => "catalog_test", // DIFFERENT UNIQUE ID
//         "name" => "კატალოგი ტესტ",
//         "loader" => array(
//             "serviceUrl" => "/custom/catalog/indexTest.php?deal_id=$dealId"
//         )
//     );

//     array_push($tabs, $catalogTabTest);
// }
