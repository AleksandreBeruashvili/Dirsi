...
<script>
    ...
    if(pathname[1] == "crm" && pathname[2] == "deal" ){
        ...
        setInterval(() => {
            ...
            // ✏️ EDIT MODE
            const editSource = document.querySelector(
                "[data-cid='SOURCE_ID'] .ui-ctl-element"
            );

            // 👁 VIEW MODE
            const viewSource = document.querySelector(
                "[data-cid='SOURCE_ID'] .ui-entity-editor-content-block-text"
            );

            if (editSource) {
                selectedSource = editSource.innerText.trim();
            } else if (viewSource) {
                selectedSource = viewSource.innerText.trim();
            }
