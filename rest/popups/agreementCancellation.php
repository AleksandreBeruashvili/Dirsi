...
<head>
    <meta charset="utf-8">
    <title>ხელშეკრულების გაუქმება</title>
    <style>
        * { box-sizing: border-box; }
        .agreement-form { padding: 14px 20px 20px; max-width: 720px; margin: 0 auto; }
        .form-group { margin-bottom: 16px; }
        label { display: block; margin-bottom: 4px; font-weight: 500; color: #333; font-size: 13px; }
        input, textarea { width: 100%; padding: 7px 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 13px; background: #fff; }
        input:focus, textarea:focus { border-color: #0286ce; outline: none; }
        .file-group { margin-bottom: 16px; }
        .file-group span { display: block; margin-bottom: 4px; font-size: 13px; color: #80868e; font-weight: 500; }
        .file-group input[type="file"] { font-size: 12px; padding: 5px; border: 1px dashed #ccc; border-radius: 4px; background: #fafafa; cursor: pointer; }
        .file-group input[type="file"]:hover { border-color: #0286ce; background: #f0f8ff; }
        .button-group { display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; border-top: 1px solid #eee; padding-top: 16px; }
        .btn { padding: 9px 22px; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: 500; }
...
<div class="agreement-form">
    <form id="agreementCancellationForm" enctype="multipart/form-data">
        <input type="hidden" id="dealId" name="dealId" value="<?= $dealId ?>">

        <div class="form-group">
            <label for="comment">კომენტარი</label>
            <textarea id="comment" name="comment" rows="4"></textarea>
            <div class="error" id="comment-error">გთხოვთ შეიყვანეთ კომენტარი</div>
        </div>
        ...
        <div class="file-group">
            ...
            <div class="error" id="agreementFile-error">გთხოვთ ატვირთოთ ხელშეკრულების ფაილი</div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function validateForm() {
            ...
            if (!$("#agreementFileText").val()) {
                showError("agreementFile", "გთხოვთ ატვირთოთ ხელშეკრულების ფაილი");
                valid = false;
            } else {
        ...
        $("#agreementCancellationForm").on("submit", function(e) {
            e.preventDefault();
            ...
            formData.append("agreementFile", $("#agreementFileText").val());

            $.ajax({
                url: "/rest/popupsservices/agreementCancellation.php",
                type: "POST",
                data: formData,
                dataType: "json",
                contentType: false,
