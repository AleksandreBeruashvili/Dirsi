...
<script>
	...
	//ბათონები და ფუნქციები
		...
		//კალკულატორი
			if(Product){
				if (((userID == 1 && url[3] == "crm" && url[4] == "deal" && url[5] == "details") && (deal["STAGE_ID"] == "1" || deal["STAGE_ID"] == "2" ||  deal["STAGE_ID"] == "3" ||  deal["STAGE_ID"] == "34"))) {
					...
					const observer = new MutationObserver(() => {
						...
						if (buttonContainer && !document.getElementById('kalkButton')) {
							...
							kalkBut.style.cssText = `
								background: linear-gradient(135deg, #6bb359ff 0%, #61dc6bff 100%);
								box-shadow: 0 4px 15px rgba(93, 210, 97, 0.4);
							`;
							kalkBut.innerHTML = `
	...
	// კონკრეტულ სთეიჯებზე გადატანის დაბლოკვა
		...
		if ( url[3] == "crm" && url[4] == "deal" && url[5] == "details" ) {
				if(deal["STAGE_ID"] == "NEW" || deal["STAGE_ID"] == "PREPARATION" ||  deal["STAGE_ID"] == "PREPAYMENT_INVOICE" ||  deal["STAGE_ID"] == "EXECUTING"){
				setTimeout(function() {
					// აიდების სია, რომლებიც უნდა დაიბლოკოს
					const blockedIds = ["FINAL_INVOICE", "1", "2", "3", "4"];
		...
		if ( url[3] == "crm" && url[4] == "deal" && url[5] == "details" ) {
				if(deal["STAGE_ID"] == "FINAL_INVOICE" || deal["STAGE_ID"] == "1" ){
				setTimeout(function() {
					// აიდების სია, რომლებიც უნდა დაიბლოკოს
					const blockedIds = [ "1", "2", "3", "4"];
					blockedIds.forEach(id => {
						const element = document.querySelector(`[data-id="${id}"]`);
						if (element) {
							element.style.pointerEvents = 'none';
